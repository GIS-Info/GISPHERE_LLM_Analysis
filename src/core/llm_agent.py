...
1. "Deadline": Extract ONLY the "application deadline" that is explicitly mentioned in the text. Convert to YYYY-MM-DD format. If deadline is not mentioned, use "Soon".
   - YEAR INFERENCE (only when the deadline gives a month/day but NO explicit year):
     * FIRST, infer the year from other dated context in the SAME posting — the intake/start term ("starting Fall 2026", "2027/2028 intake", "Spring 2026 start"), the posting/call date, review dates, or any other year the text mentions. An application deadline normally falls a few months BEFORE the stated start term, so a "January 7" deadline for a "Fall 2026" start is 2026-01-07, NOT 2027.
     * ONLY IF the posting contains no year context whatsoever, fall back to today's date ({current_date}): if the month/day has already passed this year, assume next year ({current_year + 1}); otherwise assume this year ({current_year}).
     * Do NOT roll a deadline forward into a future year when the posting's own context implies an earlier year — even if that makes the resulting deadline already in the past. Faithfulness to the posting's context beats assuming the position is still open.
