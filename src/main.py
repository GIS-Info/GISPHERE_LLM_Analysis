...
# 导入项目模块
from .core.config import setup_logging, ensure_directories, GOOGLE_SPREADSHEET_ID, check_google_credentials
...
from .core.analysis_stage import AnalysisStageManager
...
class LLMAnalysisSystem:
    ...
    def _check_environment(self) -> bool:
        ...
        # 每月首次运行时自动刷新 MODELS.md（可用模型与定价清单）
        try:
            from .tools.update_models import maybe_update_monthly
            maybe_update_monthly()
        except Exception as e:
            logger.warning(f"MODELS.md 自动更新跳过: {e}")
    ...
    def _process_single_row(self, row_index: int) -> bool:
        ...
        try:
            ...
            if not used_notes:
                from .core.utils import is_third_party_url
